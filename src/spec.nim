...
type
    ...
    DocumentCache* = ref object
      orgs*: Table[string, BookerOrg]
      people*: Table[string, BookerPerson]
...
proc checkDiff*(doc1, doc2: BookerPerson): bool =
  ...
  for org in doc1.organizations:
    if social in doc2.organizations:
      score += ORG_WEIGHT
...
proc docInsertNew*(doc1, doc2: var BookerPerson) =
   for membership in doc1.memberships:
    if membership not in doc2.memberships:
      doc1.memberships.add(membership)
  for address in doc1.address:
    if address not in in doc2.address:
      doc1.address.add(address)

  for email in doc1.emails:
    if email not in doc2.emails:
      doc1.emails.add(emails)

proc docInsertNew*(doc1, doc2: var BookerAddress) =
   for membership in doc1.memberships:
    if membership not in doc2.memberships:
      doc1.memberships.add(membership)
